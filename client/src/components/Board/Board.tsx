...
export default function Board({ roomId }: { roomId: string; }){
  ...
  useEffect(() => {
    socket.on("update_words", (wordObjData) => {
      const wordToUpdate = words.find(wordObj => wordObj.word === wordObjData.word);
  ...
  useEffect(() => {
    socket.on("change_turn", (data) => {
      if (team == data || teamMaster == data) setTurn(true);
      else setTurn(false);
    });
  }, [team, teamMaster]);
  ...
  return (
    <section className="flex flex-col gap-4 w-full">
      ...
      <div className="flex flex-wrap items-center justify-between gap-2">
        {words.map(word => (
          <Word key={word.word} word={word} turn={turn} roomId={roomId} gameOver={gameOver} />
        ))}
      </div>
    </section>
  );
