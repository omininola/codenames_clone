...
export default function Hints({ roomId }: { roomId: string }) {
  const { team, teamMaster } = UseAppContext()
  ...
  return (
    <>
      {started && (
        <div className="flex flex-col items-center gap-4 mt-2 md:mt-0 w-full md:w-2/3">
          <HintForm roomId={roomId} />

          {team !== 'none' && !teamMaster && (
...
export function Hint({
  ...
  return (
    <li
      className={`flex items-center justify-between w-full text-white py-1 rounded ${classes}`}
    >
